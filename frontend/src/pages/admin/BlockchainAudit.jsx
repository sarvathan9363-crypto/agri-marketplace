    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <span className="text-[#00684a] font-extrabold text-xs tracking-widest uppercase font-display bg-[#00ed64]/20 px-3 py-1 rounded-full inline-flex items-center gap-1.5">
            <ShieldCheck className="w-3.5 h-3.5 text-[#00684a]" /> Direct Smart Contract RPC Query
          </span>
          <h1 className="text-3xl font-black text-[#001e2b] font-display mt-2 flex items-center gap-3">
            Blockchain Audit Trail
          </h1>
          <p className="text-sm text-gray-600 mt-1 font-sans">
            Immutable, append-only payment & settlement audit records fetched directly from Kava EVM Testnet smart contract without database dependency.
          </p>
        </div>
        ...
        >
          ...
          {loading ? 'Fetching On-Chain Data...' : 'Fetch On-Chain Data'}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-1">
          <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Blockchain Network</span>
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-1">
          <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Smart Contract Address</span>
          <div className="flex items-center justify-between">
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-1">
          <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">On-Chain Payment Audits</span>
          <div className="flex items-center justify-between">
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-1">
          <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">On-Chain Settlement Audits</span>
          <div className="flex items-center justify-between">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gradient-to-r from-[#001e2b] to-[#003846] p-6 rounded-2xl text-white shadow-lg space-y-4">
          <div className="flex items-center gap-2 text-emerald-400 font-bold text-xs uppercase tracking-widest font-display">
            <Search className="w-4 h-4" /> Verify Event ID Hash On-Chain
          </div>
          <h3 className="text-lg font-bold">Smart Contract Event Verifier</h3>
          <form onSubmit={handleVerifyHash} className="flex flex-col sm:flex-row gap-3">
            <input
              type="text"
              placeholder="Paste Event ID Hash (e.g. 0xd8273e6f...)"
                  ...
                  <div className="font-mono text-xs space-y-3 mt-3 bg-black/60 p-4 rounded-xl border border-white/10 overflow-hidden shadow-inner">
                    ...
                    <div className="text-emerald-300 font-bold font-sans text-sm pb-2.5 border-b border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <span className="text-gray-400 text-xs block">Items Purchased:</span>
                          <span className="text-white font-bold">{verifyResult.event.itemsSummary || 'Agricultural Produce'}</span>
                        </div>
                      </div>
                      <div className="text-right">
                        <span className="text-gray-400 text-xs block font-sans">Total Amount:</span>
                    ...
                    <div className="space-y-2.5 pt-1 text-gray-300">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 bg-white/5 p-2.5 rounded-lg border border-white/5">
                        <span className="text-gray-400 font-sans text-xs font-semibold shrink-0">Payment ID Hash:</span>
                      ...
                      {/* Order ID Hash */}
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 bg-white/5 p-2.5 rounded-lg border border-white/5">
                        <span className="text-gray-400 font-sans text-xs font-semibold shrink-0">Order ID Hash:</span>
                        <div className="flex items-center gap-1.5 min-w-0">
                      ...
                      <div className="flex flex-col gap-2.5 bg-amber-950/50 border border-amber-500/30 p-3 rounded-xl">
                        ...
                        {verifyResult.event.sellerSplits && verifyResult.event.sellerSplits.length > 0 ? (
                          ...
                          <span className="text-xs text-amber-200/60 font-sans italic">No Seller Split Hashes recorded for this event.</span>
                        )}
                      </div>
                      ...
                      <div className="flex flex-wrap items-center justify-between text-[11px] text-gray-400 pt-1 font-sans border-t border-white/10 gap-2">
                        <span>Amount in Paise: <strong className="text-gray-200">{verifyResult.event.amountPaise}</strong></span>
                        <span>Recorded At: <strong className="text-gray-200">{new Date(verifyResult.event.recordedAt).toLocaleString()}</strong></span>
                      </div>
                    </div>
                  </div>
                ) : (
        ...
        <div className="bg-gradient-to-r from-[#002b36] to-[#004859] p-6 rounded-2xl text-white shadow-lg space-y-4">
          <div className="flex items-center gap-2 text-cyan-400 font-bold text-xs uppercase tracking-widest font-display">
            <UserCheck className="w-4 h-4" /> Buyer & Seller Hash Lookup
          </div>
          <h3 className="text-lg font-bold">Lookup User Identity by Account Hash</h3>
          <p className="text-xs text-gray-300 font-sans">
            Paste any Buyer Hash or Seller Hash (`0x...`) to reveal their full profile, contact info, and verification status.
          </p>
          <div className="flex flex-col sm:flex-row gap-3">
            <input
              type="text"
              placeholder="Paste Buyer Hash or Seller Hash (0x...)"
            ...
            >
              <UserCheck className="w-4 h-4" /> Lookup Identity
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="p-4 sm:p-6 overflow-x-auto">
          {loading ? (
            <div className="py-16 text-center text-gray-500 space-y-3">
              ...
              <p className="font-medium text-sm">Querying Kava EVM Testnet Smart Contract...</p>
            </div>
          ) : activeTab === 'payments' ? (
            data.paymentEvents.length === 0 ? (
              <div className="py-16 text-center text-gray-500">
                <Database className="w-12 h-12 mx-auto text-gray-300 mb-3" />
                <p className="font-bold text-gray-700">No payment events recorded on-chain yet</p>
                <p className="text-xs text-gray-400 mt-1">New payments processed through Razorpay will be automatically audited here.</p>
              </div>
            ) : (
              <table className="w-full text-left border-collapse min-w-[1000px]">
                <thead>
                  <tr className="text-xs font-extrabold uppercase text-gray-400 border-b border-gray-100 pb-3">
                    <th className="py-3 px-3">Event ID Hash</th>
                    <th className="py-3 px-3">Items Purchased</th>
                    <th className="py-3 px-3">Buyer Hash & Identity</th>
                    <th className="py-3 px-3">Seller Split Hashes</th>
            ...
            data.settlementEvents.length === 0 ? (
              <div className="py-16 text-center text-gray-500">
                <Database className="w-12 h-12 mx-auto text-gray-300 mb-3" />
                <p className="font-bold text-gray-700">No settlement events recorded on-chain yet</p>
                <p className="text-xs text-gray-400 mt-1">Farmer marketplace settlements will appear here when recorded.</p>
              </div>
            ) : (
              <table className="w-full text-left border-collapse min-w-[900px]">
                <thead>
                  <tr className="text-xs font-extrabold uppercase text-gray-400 border-b border-gray-100 pb-3">
                    <th className="py-3 px-3">Event ID Hash</th>
                    <th className="py-3 px-3">Settlement Hash</th>
                    <th className="py-3 px-3">Seller Identity</th>
                    <th className="py-3 px-3">Seller Payout Amount</th>
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-5 border border-gray-100 animate-in fade-in zoom-in duration-200">
            <div className="flex items-center justify-between border-b border-gray-100 pb-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-xl font-extrabold text-[#001e2b] font-display">Account Identity Lookup</h3>
                  <p className="text-xs text-gray-500 font-sans">Verified database record matching On-Chain Cryptographic Hash.</p>
                </div>
              </div>
              <button
            ...
            {accountModal.loading ? (
              <div className="py-12 text-center text-gray-500 space-y-3">
                <RefreshCw className="w-8 h-8 animate-spin mx-auto text-[#00684a]" />
                <p className="font-bold text-sm">Searching registered users and farmers...</p>
              </div>
              ...
              <div className="space-y-4">
                <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-200 text-xs font-mono text-emerald-900 break-all">
                  <strong>On-Chain Hash:</strong> {accountModal.data.accountHash}
                </div>
                ...
                <div className="bg-gray-50 p-4 rounded-2xl space-y-3 font-sans text-sm">
                  ...
                  {accountModal.data.farmer ? (
                    <>
                      ...
                      <div className="flex justify-between border-b pb-2">
                        <span className="text-gray-500 flex items-center gap-1.5"><Tractor className="w-4 h-4 text-gray-400" /> Farm / Business:</span>
                      ...
                      <div className="flex justify-between border-b pb-2">
                        <span className="text-gray-500 flex items-center gap-1.5"><Phone className="w-4 h-4 text-gray-400" /> Mobile:</span>
              ...
              <div className="py-8 text-center space-y-2">
                ...
                <p className="font-bold text-gray-800">No Account Found</p>
